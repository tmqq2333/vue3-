...
export default defineComponent({
  ...
  setup(props) {
    ...
    const state = reactive({
      ...
      tabs: props.tabs && props.tabs.length ? props.tabs : [defaultTab]
    });
    watch(
      () => state.tabs,
      (res) => {
    ...
    emits.on(EMitt.OnPushMenuToTabs, (route) => {
      ...
      const tabKeys= arrayToObject(state.tabs, "value", () => 1);
      if (!tabKeys[path]) {
    ...
    const updateClosedTabs = (closedTabs, isTransform = true) => {
      if (isTransform) {
