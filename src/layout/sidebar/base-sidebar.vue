...
export default defineComponent({
  ...
  setup(props) {
    ...
    const defaultMenus = toValidRoutes(props.menus ?? store.state.routes);
    ...
    const initComputeSidebarLayout = (width) => {
    ...
    watch(
      () => props.menus,
      (vl) => {
        const ms = toValidRoutes(vl ? vl : store.state.routes);
        state.menus = ms;
        state.rawMenus = ms;
      }
    );
    ...
    const computeSidebarLayout = (windowWidth) => {
...
<template>
  ...
  >
    <sidebar-menus-items :className="state.popClassName" ismix  :menus="state.menus" :hiddenIndex="state.hiddenIndex"></sidebar-menus-items>
  </el-menu>
</template>
